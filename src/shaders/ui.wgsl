// Corrected copy of bevy_ui 0.14.2's `render/ui.wgsl` (MIT / Apache-2.0,
// (c) Bevy contributors).  Installed over the built-in shader by
// `override_ui_shader` in src/lib.rs.
//
// The one change is in `antialias()`: upstream 0.14 calls
// `clamp(0.0, 1.0, x)` — WGSL's signature is `clamp(e, low, high)`, so that
// reads "clamp 0.0 into [1.0, x]", undefined whenever x < 1.  Vulkan/Metal
// drivers happen to evaluate it as `min(1, x)` and borders come out right by
// accident; GL/ANGLE/SwiftShader (i.e. WebGL2) don't, and every bordered
// node is painted solid in its border colour.  Fixed upstream after 0.14.
// Drop this file once the game moves to a Bevy with the fix.
...
fn antialias(distance: f32) -> f32 {
    ...
    return clamp(0.5 - 2.0 * distance, 0.0, 1.0);
}
